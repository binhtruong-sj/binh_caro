...
int search_depth = 5;
int search_width = 5;
// setBreastDepth[][2] = { { 3, 14 }, { 5, 8 }, { 10, 6 }, { 15, 4 } };
int setWidthDepth[][2] = { { 14, 6 }, { 10, 8 }, { 6, 8 }, { 6, 8 }, { 4, 10 } };
//int setWidthDepth[][2] = { { 14,8 }, { 10, 8 }, { 6, 10 }, { 6, 10 }, { 4, 12 } };
int lowestDepth = 14;
#define MINSCORE 4
		...
		for (int j = 0; j < 2; j++) {
			...
			for (int dir = East; dir < West; dir++) {
				...
				if ((debugThis && (debugCell || debugScoring))) {
					cout << "CurrPlay=" << currPlay << " " << astar.Xlines[dir]
	...
	if (highestConnected) {
		...
		for (int i = 0; i < (int) (bestScoreArray.size()); i++) {
			...
			if ((abs(bestScoreArray[i].connectedOrCost) >= abs(highestConnected))
					|| ((highestConnected == 3)
							&& (abs(bestScoreArray[i].connectedOrCost) == 2))
					|| ((highestConnected == -3)
							&& (abs(bestScoreArray[i].connectedOrCost) == 2))) {
...
hist caro::printTracerToBoard(int currPlay, const scoreElement& bestScore,
		tracer* headTracer) {
	hist histArray;
	// print trace from previous level, hence nextPlay instead of currPlay
	headTracer->next->extractTohistArray(currPlay, bestScore.cellPtr,
			histArray);
	print(histArray);
	cout << endl;
	return histArray;
...
scoreElement caro::evalAllCell(int currPlay, int in_NodeCnt, const int in_depth,
		int min_depth, bool isMax, aScore alpha, aScore beta, bool debugThis,
		bool &redo, traceCell * const callerTrace, tracer * const headTracer) {
	caro backup(15);
	...
	if (in_depth == -1) {
		...
		thisNodeCnt = createNodeList(thisNodeCnt, in_NodeCnt, bestScoreArray,
				highestConnected, bestScore);
		for (int i = 0; i < thisNodeCnt; i++) {
			oneWork onePath;
			// This code is only 1 depth as supposed to 2 depth
			cout << "I=" << i << " " << bestScoreArray[i] << endl;
#ifdef oneD
			onePath.cptr = bestScoreArray[i].cellPtr;
			onePath.val = currPlay;
			workQ.addWork(onePath);
#else
			onePath.cptr[0] = headTracer->savePoint.cellPtr;
			onePath.val[0] = nextPlay;
			onePath.cptr[1] = bestScoreArray[i].cellPtr;
			onePath.val[1] = currPlay;
			workQ.addWork(onePath);
#endif
		}
		cout << endl;
		...
		int swidth = setWidthDepth[highestConnected][0];
		int sdepth = setWidthDepth[highestConnected][1] + deltaDepth;
		in_NodeCnt = swidth;
		thisNodeCnt = createNodeList(thisNodeCnt, in_NodeCnt, bestScoreArray,
				highestConnected, bestScore);
		...
		if (in_depth > (maxDepth - sdepth)) {
			...
			if (min_depth < lowestDepth)
				min_depth = lowestDepth;
		}
		...
		for (int pass = passStart; pass < 2; pass++) {
			...
			if (debugThis || debugScoringd || debugScoringAll
				...
				if (debugThis) {
					cout << "w=" << swidth << ", sd=" << sdepth << "maxD="
							<< maxDepth << ", in_depth = " << in_depth
			...
			if ((terminated == 0) && (in_depth > min_depth)) {
				if (isMax)
					...
					for (int i = 0; i < thisNodeCnt; i++) {
						...
						tracerArrayPtr[i]->savePoint = bestScoreArray[i];
						...
						tracerArrayPtr[i]->evalCnt = evalCnt;
						...
						if(isMax) {
							bestScore = asMAX(found_better, returnScore, bestScore);
							if(found_better) {
								alpha = asMAX(newAlpha, bestScore, alpha);
							}
						} else {
							bestScore = asMIN(found_better, returnScore,bestScore);
							if(found_better) {
								beta = asMIN(newBeta, bestScore, beta);
							}
						}
						...
						if ((alpha.greaterValueThan(beta))) {
							if(debugThis)
							cout << "Alpha " << alpha << " greater tha beta " << beta << endl;
							terminated = 1;
							thisNodeCnt = i+1;
						}
						...
						if(in_depth == ( maxDepth -1)) {
							...
							cout << "T=" << tid << " i=" << i << bestScore
									<< " Eval-" << evalCnt
									<< "  al=" << alpha << " be" << beta << endl;
					...
					if (debugThis || (in_depth == maxDepth)) {
						...
						do {
							for (int i = 0; i < thisNodeCnt; i++) {
								...
								// 	tracerArrayPtr[i]->print(ofile) ;
								hist histArray;
								tracerArrayPtr[i]->extractTohistArray(currPlay,
										bestScoreArray[i].cellPtr, histArray);
								print(histArray);
							}
							cout << " bestScore=" << bestScore << endl;
							printTracerToBoard(currPlay, bestScore, headTracer);
							if (debugThis) {
								cout << "Redo?" << endl;
								cin >> redoCh;
							} else {
